...
def sort_neighbors(map, node, temp):
    ...
    for i in range(len(temp)):
        ...
        neighbors.append( ( (temp[i][0], temp[i][1]), battle(node, child) ) )
    ...
    for i in range(len(map)):
        for j in range(len(map[i])):
            if map[i][j] == 4 or map[i][j] == 5 or map[i][j] == 6:
                ...
                scores.append(MiniMax(map, node, node, 9, 1, -math.inf, math.inf))  # This begins the min max search with alpha beta pruning
    return scores
...
def MiniMax(map, node, child, depth, playerNum, a, b):
        ...
        neighbors = sort_neighbors(map, node, neighbors_temp)
